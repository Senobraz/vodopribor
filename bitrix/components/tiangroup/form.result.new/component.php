...
/**
 *  Изменения от стандартного form.result.new
 *  - законентированны все LocalRedirect для EDIT_URL, SUCCESS_URL, LIST_URL
 *  - добалена проверка кодировки if( LANG_CHARSET == 'windows-1251' )
 *  с изменением $_REQUEST'а
 *  
 * 
 */
	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
		...
		if (($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) && (strlen($_REQUEST["web_form_submit"])>0 || strlen($_REQUEST["web_form_apply"])>0))
		{
			...
			if( LANG_CHARSET == 'windows-1251' )
			{
				...
				{
					$_REQUEST[$key] = mb_convert_encoding($value, 'Windows-1251', 'UTF-8');
				}
			}
